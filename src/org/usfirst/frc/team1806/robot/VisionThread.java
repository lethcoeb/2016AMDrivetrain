...
public class VisionThread extends Thread {
	...
	public VisionThread() {
		camera = new AxisCamera("10.8.6.14");
		// 10.18.6.14
		// 10.1.91.100
	...
	public void run() {
		while (1 == 1) {
			if (Robot.states.VisionTrackingTracker == States.VisionTracking.ON) {
				...
				if (numParticles > 0) {
					...
					boolean isTote =  scores.Area > SCORE_MIN;
					...
					SmartDashboard.putNumber("angle to goal", deltaAngle);
						...
						SmartDashboard.putNumber("angle to goal", 99999);
				...
				// FIXME: currently sets image to something other than the
				// actual picture

				try {
					VisionThread.sleep(50);
